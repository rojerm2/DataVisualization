      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (ctx) =>
                `${ctx.raw.label}: ${ctx.raw.y.toFixed(2)} (GDP: ${ctx.raw.x.toFixed(2)})`,
            },
            titleColor: "#fff",
            bodyColor: "#fff",
            backgroundColor: "#333",
...
function loadBarChart() {
  const top15 = [...allDataPoints].sort((a, b) => b.y - a.y).slice(0, 15);
...
function loadBottomBarChart() {
  const bottom15 = [...allDataPoints].sort((a, b) => a.y - b.y).slice(0, 15);
